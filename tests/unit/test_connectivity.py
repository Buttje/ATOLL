...
class TestSetServerCommand:
    ...
    async def test_setserver_command_failure_reverts(self):
        ...
        app.config_manager.ollama_config = OllamaConfig(base_url=original_url, port=original_port)
        app.config_manager.mcp_config = Mock(servers={})

        app.agent = Mock()
        app.agent.ollama_config = OllamaConfig(base_url=original_url, port=original_port)
        app.agent._create_llm = Mock(return_value=Mock())
