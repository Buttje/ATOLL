...
class TestMCPServerManagerExtended:
    ...
    async def test_connect_and_discover_with_tools(self):
        ...
        config = MCPConfig(
            servers={"test": MCPServerConfig(transport="stdio", command="python", args=["test.py"])}
        )
    ...
    async def test_connect_and_discover_connection_failure(self):
        ...
        config = MCPConfig(servers={"test": MCPServerConfig(transport="stdio", command="python")})
    ...
    async def test_execute_tool(self):
        ...
        manager.tool_registry.register_tools(
            "test_server", [{"name": "test_tool", "description": "Test tool"}]
        )
        ...
        result = await manager.execute_tool(
            server_name="test_server", tool_name="test_tool", arguments={"arg": "value"}
        )
    ...
    async def test_execute_tool_server_not_found(self):
        ...
        with pytest.raises(ValueError, match="Server 'nonexistent' not found"):
            await manager.execute_tool(
                server_name="nonexistent", tool_name="test_tool", arguments={}
            )
