        ...
        if cmd == "quit" or cmd == "exit":
            ...
            if len(parts) == 1:
                ...
                if subcmd == "server" and len(parts) == 3:
                    ...
                    self.ui.display_error(f"Invalid help command. Use 'help', 'help server <name>', or 'help tool <name>'")
            else:
                self.display_help()
...
"""
    ...
    def display_server_help(self, server_name: str) -> None:
        ...
        if not client:
            self.ui.display_error(f"Server '{server_name}' not found. Use 'servers' to list available servers.")
            return
        ...
        if server_config:
            ...
            if server_config.env:
                print(self.colors.user_input(f"  Environment:"))
        ...
        if server_tools:
            ...
            print()
            print(self.colors.info(f"Use 'help tool <name>' for detailed tool information."))
        else:
    ...
    def display_tool_help(self, tool_name: str) -> None:
        ...
        if not tool_info:
            self.ui.display_error(f"Tool '{tool_name}' not found. Use 'tools' to list available tools.")
            return
        ...
        print(self.colors.reasoning(f"  This tool can be invoked by the AI agent when processing prompts."))
        print(self.colors.reasoning(f"  You can ask questions that require this tool's functionality."))
        print()
    ...
    def display_servers(self) -> None:
        ...
        if servers:
            ...
            for server in servers:
                ...
                status_color = self.colors.answer_text if client and client.connected else self.colors.error
